...
- **Wide-tier statusline emits 5 rows (DEC-016 amendment, TODO-047).** Silhouette rows 1–4 wander as a composed creature; HUD glyphs pin on a dedicated row 5. The `CompactFrame.rows.length ≤ 3` invariant constrains scene data (`Scene.compact[]`) only, NOT assembled stdout. Wide-tier silhouettes (`SILHOUETTES[species][stage].wide`) stay at exactly 4 rows.
- **Wander is two regimes, not one.** `useWander` (TUI, stateful, 2 Hz, 2-tick edge hold in `src/render/useWander.ts`) and `computeWanderX` (statusline, pure-time-derived, 1 Hz, 1-tick edge hold in `src/render/compact.ts`) are intentionally **not** unified — different runtime regimes by design. Both produce ~1 s of visible pause at edges. Do not "fix" the perceived asymmetry without revisiting DEC-016 statelessness. The cross-consistency invariant is: every `SceneKey` in `ONE_SHOT_SCENES` maps to a `SceneId` NOT in `AMBIENT_SCENES` (unit-tested in `src/render/compact-wander.test.ts`).
